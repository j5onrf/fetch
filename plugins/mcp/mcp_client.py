...
def load_dotenv() -> None:
    """Auto-loads keys from ~/.config/fetch/.env into os.environ (zero external dependencies)."""
    env_paths = [
        os.path.expanduser("~/.config/fetch/.env"),
...
class MCPSession:
    ...
    def __enter__(self):
        self.proc = subprocess.Popen(
            ...
            stderr=subprocess.DEVNULL,  # Prevents 64KB pipe buffer deadlocks on npm/npx downloads
            text=True,
            bufsize=1,
            env=self.env,
        )
    ...
    def _send_rpc(self, method: str, params: dict[str, Any] | None = None, timeout: float = 5.0) -> dict[str, Any] | None:
    ...
    def _initialize(self) -> None:
        init_params = {
            ...
            "capabilities": {},
            "clientInfo": {"name": "fetch-mcp", "version": "1.0"},
        }
        self._send_rpc("initialize", init_params, timeout=6.0)
        self._send_notification("notifications/initialized")

    def list_tools(self) -> list[dict[str, Any]]:
        res = self._send_rpc("tools/list", timeout=5.0)
        return res.get("tools", []) if res else []

    def call_tool(self, name: str, arguments: dict[str, Any]) -> str:
        res = self._send_rpc("tools/call", {"name": name, "arguments": arguments}, timeout=15.0)
        if not res:
